...
#ifdef COREXY
 #define A_MOTOR X_AXIS // Must be X_AXIS	必须是X_AXIS
 #define B_MOTOR Y_AXIS // Must be Y_AXIS   必须是Y_AXIS
#endif
...
#define bit(n) (1 << n) 
  ...
  #define bit_true_atomic(x,mask) 	{  cli();(x) |= (mask);  sei();}	
  #define bit_false_atomic(x,mask) 	{  cli();(x) &= ~(mask); sei();}	
  #define bit_toggle_atomic(x,mask) {  cli();(x) ^= (mask);  sei();}	
  #define bit_true(x,mask) 			(x) |= (mask)	
  #define bit_false(x,mask) 		(x) &= ~(mask)	
  #define bit_istrue(x,mask) 		((x & mask) != 0)	
  #define bit_isfalse(x,mask) 		((x & mask) == 0)	

...
// 步进电机状态初始化. 循环只可在st.cycle_start标记使能时开启. 启动和限位部分代码会调用这个函数但是不会开启循环.
void st_wake_up(void) 
{
  // Enable stepper drivers.使能步进电机驱动
...
void st_go_idle(void) 
{
  bool pin_state;
  // Disable Stepper Driver Interrupt. Allow Stepper Port Reset Interrupt to finish, if active.
  HW_TIM_DriverInterrupt_Disable(); 					//关闭定时器
  busy = false;
  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) { pin_state = !pin_state; } // Apply pin invert.
  if (pin_state) { HW_GPIO_Write(STEPPERS_DISABLE_GPIO,STEPPERS_DISABLE_GPIO_PIN,1);}
  else { HW_GPIO_Write(STEPPERS_DISABLE_GPIO,STEPPERS_DISABLE_GPIO_PIN,0);}
}
#endif		//end of CPU_MAP_STM32F10X
...
#if defined(CPU_MAP_STM32F10X)
void TIM3_IRQHandler(void)   //TIM3中断:The Stepper Driver Interrupt
{
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)  //检查TIM3更新中断发生与否
	{
		TIM_ClearITPendingBit(TIM3, TIM_IT_Update  );  //清除TIM3更新中断标志 
		if (busy) { return; } // The busy-flag is used to avoid reentering this interrupt	 忙碌标志用于防止重复进入此中断
		...
		st.step_bits = (STEP_PORT & ~STEP_MASK) | st.step_outbits; // Store out_bits to prevent overwriting.
#else  // Normal operation
		HW_GPIO_Write(STEP_GPIO,X_STEP_GPIO_PIN,bit_istrue(st.step_outbits,bit(X_STEP_BIT)));
		HW_GPIO_Write(STEP_GPIO,Y_STEP_GPIO_PIN,bit_istrue(st.step_outbits,bit(Y_STEP_BIT)));
		HW_GPIO_Write(STEP_GPIO,Z_STEP_GPIO_PIN,bit_istrue(st.step_outbits,bit(Z_STEP_BIT)));
#endif  
      ...
      // Initialize step segment timing per step and load number of steps to execute.    
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
        HW_TIM_DriverInterrupt_ValueConfig(8,st.exec_segment->cycles_per_tick>>1);		//使能AMASS算法就使用固定值分频【16M不分频】
#else
				// With AMASS is disabled, set timer prescaler for segments with slow step frequencies (< 250Hz).
        HW_TIM_DriverInterrupt_ValueConfig(st.exec_segment->prescaler,st.exec_segment->cycles_per_tick);		//不使能AMASS算法就使用硬件分支产生分频
#endif
...
#if defined(CPU_MAP_STM32F10X)
void TIM4_IRQHandler(void)   //TIM4中断:The Stepper Port Reset Interrupt
{
...
//  DIRECTION_PORT = (DIRECTION_PORT & ~DIRECTION_MASK) | dir_port_invert_mask;
  HW_GPIO_Write(DIRECTION_GPIO,X_DIRECTION_GPIO_PIN,bit_istrue(dir_port_invert_mask,bit(X_DIRECTION_BIT)));
  HW_GPIO_Write(DIRECTION_GPIO,Y_DIRECTION_GPIO_PIN,bit_istrue(dir_port_invert_mask,bit(Y_DIRECTION_BIT)));
  HW_GPIO_Write(DIRECTION_GPIO,Z_DIRECTION_GPIO_PIN,bit_istrue(dir_port_invert_mask,bit(Z_DIRECTION_BIT)));
...
void stepper_init(void)
{
  ...
  HW_GPIO_Init_Out(DIRECTION_GPIO_CLK,DIRECTION_GPIO,X_DIRECTION_GPIO_PIN);
  HW_GPIO_Init_Out(DIRECTION_GPIO_CLK,DIRECTION_GPIO,Y_DIRECTION_GPIO_PIN);
...
void st_prep_buffer(void)
{
  ...
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
    ...
    */
    ...
    cycles = ceil((1000000*inv_rate*60)*TICKS_PER_MICROSECOND ); // (cycles/step)    

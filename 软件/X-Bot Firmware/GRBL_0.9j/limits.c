...
void limits_init(void) 
{
  ...
  LIMIT_DDR &= ~(LIMIT_MASK); // Set as input pins	 设置为输入引脚

  #ifdef DISABLE_LIMIT_PIN_PULL_UP
    LIMIT_PORT &= ~(LIMIT_MASK); // Normal low operation. Requires external pull-down.	 常低电平操作。需要外部下拉。
  #else
    LIMIT_PORT |= (LIMIT_MASK);  // Enable internal pull-up resistors. Normal high operation. 使能内部上拉电阻。 常高电平操作。
  #endif

  if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
    LIMIT_PCMSK |= LIMIT_MASK; // Enable specific pins of the Pin Change Interrupt	   使能引脚电平变化中断的特定引脚
    PCICR |= (1 << LIMIT_INT); // Enable Pin Change Interrupt	 使能引脚电平变化中断
  } else {
    limits_disable(); 
  }
    ...
    MCUSR &= ~(1<<WDRF);
    WDTCSR |= (1<<WDCE) | (1<<WDE);
    WDTCSR = (1<<WDP0); // Set time-out at ~32msec.	设置溢出时间约32ms
  #endif
#endif		//end of CPU_MAP_ATMEGA328P & CPU_MAP_ATMEGA2560

#if defined(CPU_MAP_STM32F10X)
   	HW_GPIO_Init_In(LIMIT_GPIO_CLK,LIMIT_GPIO,X_LIMIT_GPIO_PIN); 	//设置为输入引脚
	HW_GPIO_Init_In(LIMIT_GPIO_CLK,LIMIT_GPIO,Y_LIMIT_GPIO_PIN);	//设置为输入引脚
	HW_GPIO_Init_In(LIMIT_GPIO_CLK,LIMIT_GPIO,Z_LIMIT_GPIO_PIN);	//设置为输入引脚
  #ifdef DISABLE_LIMIT_PIN_PULL_UP
    HW_GPIO_Write(LIMIT_GPIO,X_LIMIT_GPIO_PIN,0);	//低电平
	HW_GPIO_Write(LIMIT_GPIO,Y_LIMIT_GPIO_PIN,0);	//低电平
	HW_GPIO_Write(LIMIT_GPIO,Z_LIMIT_GPIO_PIN,0);	//低电平
  #else
	...
	if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
		...
		HW_EXTI_Enable(LIMIT_INT_CHANNEL,0,0);	//使能中断
  	} else {
    	limits_disable(); 
  	}
  #ifdef ENABLE_SOFTWARE_DEBOUNCE	//使能软件去抖
	HW_Debounce_Init();		//去抖定时器初始化
  #endif
...
#if defined(CPU_MAP_STM32F10X)
	HW_EXTI_Disable(LIMIT_INT_CHANNEL,0,0);	
#endif		//end of CPU_MAP_STM32F10X
}
...
uint8_t limits_get_state(void)
{
  ...
  return(limit_state);
	...
	uint8_t limit_state = 0;
	uint8_t idx;
	uint8_t pin = (uint8_t)(((HW_GPIO_Read(LIMIT_GPIO,X_LIMIT_GPIO_PIN)<<X_LIMIT_BIT)|
							(HW_GPIO_Read(LIMIT_GPIO,Y_LIMIT_GPIO_PIN)<<Y_LIMIT_BIT)|
							(HW_GPIO_Read(LIMIT_GPIO,Z_LIMIT_GPIO_PIN)<<Z_LIMIT_BIT))& 
							LIMIT_MASK);
  #ifdef INVERT_LIMIT_PIN_MASK
	pin ^= INVERT_LIMIT_PIN_MASK;
  #endif
...
// your e-stop switch to the Arduino reset pin, since it is the most correct way to do this.
  ...
  // Upon limit pin change, enable watchdog timer to create a short delay. 	 
	...
	{     
		EXTI_ClearITPendingBit(X_LIMIT_INT_LINE|Y_LIMIT_INT_LINE|Z_LIMIT_INT_LINE); 
		if (sys.state != STATE_ALARM) { 
      		if (!(sys_rt_exec_alarm)) {
        	#ifdef HARD_LIMIT_FORCE_STATE_CHECK
          	// Check limit pin state.  
  ...
  // Upon limit pin change, enable watchdog timer to create a short delay. 	 
	...
	void TIM1_UP_IRQHandler(void)
	{
		if(TIM_GetITStatus(TIM1,TIM_IT_Update)!=RESET)
		{
			...
			if (sys.state != STATE_ALARM) {  // Ignore if already in alarm state.	 忽略已经处于的报警状态
	      		if (!(sys_rt_exec_alarm)) {
	        		// Check limit pin state.  检查限位引脚状态
	        		if (limits_get_state()) {
	         		 mc_reset(); // Initiate system kill.	  启动系统终止
	         		 bit_true_atomic(sys_rt_exec_alarm, (EXEC_ALARM_HARD_LIMIT|EXEC_CRITICAL_EVENT)); // Indicate hard limit critical event   指示硬件限位的关键事件
	        		}
	      		}  
	    	}
		}
...
void limits_go_home(uint8_t cycle_mask) 
{
  ...
  do {
    ...
    plan_reset(); 
	// Reset planner buffer to zero planner current position and to clear previous motions.
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    ...
    if (cycle_mask & bit(idx)) {
      ...
      #ifdef COREXY
        if (idx==X_AXIS) { 
          off_axis_position = system_convert_corexy_to_y_axis_steps(sys.position);
          sys.position[A_MOTOR] = set_axis_position + off_axis_position;
          sys.position[B_MOTOR] = set_axis_position - off_axis_position;          
        } else if (idx==Y_AXIS) {
          off_axis_position = system_convert_corexy_to_x_axis_steps(sys.position);

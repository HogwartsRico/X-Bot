...
void memcpy_to_eeprom_with_checksum(unsigned int destination, char *source, unsigned int size) {  //带校验和的向EEPROM中写入数据
...
int memcpy_from_eeprom_with_checksum(char *destination, unsigned int source, unsigned int size) {  //带校验和的从EEPROM中读出数据
